...
module.exports = async (indexName, graphql) => {
    ...
    if (filteredPagination.length === 0) {
        debug(`No pagination config with the index name "${indexName}" was found. Returns 8 by default.`);
        return 8;
    } else {
        if (filteredPagination.length > 1) {
            debug(`Multiple pagination config with the index name "${indexName}" were found: ${filteredPagination}. Returns the first one.`);
        }
